...
class KleeProcessor:
    def __init__(self, source_code_path, llm):
        ...
        self.bc_file = None
        self.klee_output_dir = "./klee-last"
    ...
    def generate_llvm_ir(self, output_name="output"):
        ...
        clang_command = f"clang -emit-llvm -I /snap/klee/10/usr/local/include -g -fsanitize=signed-integer-overflow -fsanitize=undefined -fsanitize=signed-integer-overflow -c -o {self.bc_file} {self.klee_friendly_path}"
        subprocess.run(clang_command, shell=True, check=True)

        print(f"LLVM bitcode generated: {self.bc_file}")

    def run_klee(self):
        klee_command = f"klee --libc=uclibc --posix-runtime {self.bc_file}"
